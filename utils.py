...
class Params:
    ...
    def __init__(self, json_path, input_lang, output_lang):
        self.update(json_path)
        self.load_vocab(input_lang, output_lang)
    ...
    def load_vocab(self, input_lang, output_lang):
        # add device information to the the params
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        # add <sos> and <eos> tokens' indices used to predict the target sentence
        params = {'input_dim': input_lang.vocab_size, 'output_dim': output_lang.vocab_size,
                  'sos_idx': output_lang.word_to_index.get("<sos>"), 'eos_idx': output_lang.word_to_index.get("<eos>"),
                  'pad_idx': output_lang.word_to_index.get("<pad>"),'device': device}
