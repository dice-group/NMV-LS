...
def prepareData(lang1, lang2, data_dir, max_vocab_size=50000, 
    ...
    train_pairs = filterPairs(train_pairs, trim)
    test_pairs = filterPairs(test_pairs, trim) 
    val_pairs = filterPairs(dev_pairs, trim) 
...
def train_and_test(epochs, val_eval_every, plot_every, learning_rate, lr_schedule, train_pairs, val_pairs, input_lang, output_lang, batch_size, val_batch_size, encoder, decoder, loss_criterion, trim, save_weights, create_txt, print_to, output_file_name, criterion, clip, optimizer, regularization):
    times = []
    arEpochs = []
    ...
    for i in range(1,epochs+1):
        ...
        if optimizer=="Adam":
          ...
          if regularization==True:
            encoder_optimizer = optim.SGD(encoder.parameters(), lr=learning_rate, weight_decay=1e-5)
            decoder_optimizer = optim.SGD(decoder.parameters(), lr=learning_rate, weight_decay=1e-5)
          else:
...
def main(clip, mode, max_length, data_dir, n_epoch, bidirectional, optimizer, batch_size, regularization, lr):
    create_txt=True
    ...
    if mode=="train" or mode=="all":
        train_and_test(epochs, test_eval_every, plot_every, learning_rate, lr_schedule, 
                   ...
                   test_batch_size, encoder, decoder, criterion, trim, save_weights, create_txt, print_to, output_file_name, criterion, clip, optimizer, regularization)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--regularization', type=bool, default=False, help="Regularization")
    parser.add_argument('--lr', type=float, default=0.1, help="Leaning rate")


    args = parser.parse_args()
    #print(args)
    main(args.clip, args.mode, args.max_length, args.directory, args.n_epoch, args.bidirectional, args.optimizer, args.batch_size, args.regularization, args.lr)